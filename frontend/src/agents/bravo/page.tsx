...
import { streamBravoChat } from "./api"
import type { BravoMessage } from "./types"

const initialMessages: BravoMessage[] = [{ role: "assistant", content: "Bravo ready." }]

export function BravoPage() {
  // 当前页面很小,hook 和小展示函数留在本文件;超过单人可读范围时再拆。
  const chat = useBravoChat()

  return (
    <section className="flex min-h-[720px] flex-col py-5">
      <div className="mb-4">
        <h2 className="text-lg font-semibold">Bravo</h2>
        <p className="text-sm text-muted-foreground">Owned by the Bravo developer.</p>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto rounded-md border p-4">
        <div className="space-y-4">
          ...
          {chat.isSending && chat.messages.at(-1)?.content === "" ? (
            <div className="text-sm text-muted-foreground">Bravo streaming...</div>
          ) : null}
        </div>
      </div>
...
function useBravoChat() {
  const [messages, setMessages] = useState<BravoMessage[]>(initialMessages)
  ...
  const sendMessage = useCallback(
    async (event?: { preventDefault: () => void }) => {
      ...
      try {
        const reply = await streamBravoChat({
        ...
        setMessages((current) =>
          current.map((message, index) =>
            index === assistantIndex
              ? { role: "assistant", content: reply || "Bravo returned no content." }
              : message,
          ),
        )
      } catch (err) {
        setError(err instanceof Error ? err.message : "bravo chat request failed")
...
function MessageBubble({ message }: { message: BravoMessage }) {

...
| 无状态 | 后端不存历史；前端每轮带**完整 `messages`**（`docs/api-contract.md`） | **偏好记忆只能每轮从 `messages` 重建**（见 §5.1），不能依赖跨请求内存 |
...
| 依赖 | 仅 `fastapi/uvicorn/openai/python-dotenv` + stdlib | 维持零新增依赖 |
...
显式状态对象（串讲 §1"显式数据存哪"，全是单轮内的普通数据对象，不跨请求持久化、不藏在对话历史）：`Slots` / `Preferences` / `Plan` / `Itinerary`。

---

## 5. 关键设计决策

### 5.1 偏好记忆：无状态后端下"每轮重建显式对象"（本方案最关键的适配）

**约束**：后端无状态 + 前端每轮带完整 `messages` ⇒ "对话内记忆"的唯一可靠载体就是 `messages`。

**决策**：`preferences.extract(messages)` 每轮跑一次，从全部历史抽出**显式 `Preferences` 对象**（忌口/预算档/节奏/兴趣/避免地点），供下游按字段读取。

- 这同时满足串讲两条要求：① 它是"显式存储"（结构化对象，下游按字段读，**不靠模型隐式重读原始历史**）；② 不破坏无状态契约（不需要 session id、不需要进程级内存）。
- **取舍**：放弃了 lesson7 的进程级 `Memory.items` 写法——它在无状态 HTTP 下无法跨请求存活，且契约无会话标识。每轮重抽是被契约逼出来的唯一干净解。
- **已知边界（串讲 §3/§5）**：记忆随对话增长，全量注入终会撑爆上下文。学习阶段消息量小，可接受；**截断/检索列为未来项**。
- 对应 PRD Open Question 2 默认值（仅对话内记忆）——本方案天然落在该默认。
...
| 跨会话持久化记忆、地图可视化、行程导出 | PRD §11 未来项 |
| 任何新框架 / 新依赖、markdown 渲染库 | 维持零新增依赖；行程表用纯文本时间轴清单，前端 `whitespace-pre-wrap` 直显 |
